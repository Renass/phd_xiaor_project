...
LOAD_WEIGHTS = 'renas_10_flava7_real'
SAVE_WEIGHTS = 'renas_10_flava7_real'
...
class Renas10forTrain(torch.nn.Module):
    def __init__(self, device):
        ...
        self.vlm_model = FlavaModel.from_pretrained("facebook/flava-full")
    ...
    def annot_forward(self, act_vocab_im, act_vocab_prompt, act_vocab_map):
        with torch.no_grad():
            ...
            for i, im_i in enumerate(act_vocab_im):
                ...
                #act_vocab_token.append(outputs['multimodal_output']['pooler_output'])     
                act_vocab_token.append(outputs['multimodal_output']['last_hidden_state'][: ,0, :]) 
    ...
    def forward(self, batch, act_vocab_token):
        ...
        for i, im_i in enumerate(im):
            ...
            #state.append(outputs['multimodal_output']['pooler_output'])
            state.append(outputs['multimodal_output']['last_hidden_state'][:, 0, :])
