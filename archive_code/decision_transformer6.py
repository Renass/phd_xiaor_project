...
def train_inference_thread():
    ...
    while not rospy.is_shutdown():
        ...
        while traj_buffer.gather == True:
            if len(traj_buffer.traj) > 0:
                if len(traj_buffer.traj[-1]) >  0:
                    ...
                    #print("states tensor",states_tensor.device)
                    #print("mobilenetv2",mobilenetv2.device)
                    with torch.no_grad():
        ...
        for local_epoch in range(EPOCH_PER_TRAIN):
            ...
            all_states_tensor = all_states_tensor.to(device)
            all_actions_tensor = all_actions_tensor.to(device)
            all_returns_tensor = all_returns_tensor.to(device)
            all_returns_to_go = all_returns_to_go.to(device)
            timesteps_sequence = timesteps_sequence.to(device)
            attention_mask = attention_mask.to(device)
