...
from transformers import BertModel, BertTokenizer, ViTFeatureExtractor, ViTModel, ViTImageProcessor
...
1.Image encoding with  Vision Transformer
...
class Renas(torch.nn.Module):
    def __init__(self, device):
        ...
        self.fc2 = torch.nn.Sequential(
            torch.nn.Linear(768, 768*4),
            torch.nn.GELU(),
            torch.nn.LayerNorm(768*4),
            torch.nn.Linear(768*4, 2))
    ...
    def forward(self, states_tensor, prompt):
        ...
        visual_tokens = self.vision_processor.preprocess(images= states_tensor, return_tensors= 'pt', padding = True).to(self.device)
        del states_tensor
        visual_tokens = self.vision_model(**visual_tokens).last_hidden_state
